...
class StatisticalProfiler:
    ...
    # -- category-specific sub-profilers ------------------------------------

    def _profile_numeric(
        self, non_null: list,
    ) -> tuple[
        DistributionParameters | None,
        ValueRange | None,
        Percentiles | None,
        ...
        Returns:
            Tuple of (distribution, value_range, percentiles, frequency,
            outlier_count, outlier_boundaries).
        """
        ...
        return distribution, value_range, percentiles, frequency, outlier_count, outlier_boundaries

    def _profile_categorical(
        self, non_null: list,
    ) -> tuple[DistributionParameters, FrequencyDistribution]:
        """Profile categorical column data.

        Returns:
            Tuple of (distribution, frequency).
        """
        ...
        if len(cat_counts) >= 2:
            try:
                ...
                _logger.debug("chi-square test failed for categorical column")
    ...
    def _profile_temporal(
        ...
        return (
            self._fit_distribution(arr),
            self._compute_value_range(arr),
            self._compute_percentiles(arr),
            self._compute_frequency_histogram(arr),
        )
    ...
    def _profile_column(
        ...
        try:
            distinct_values = {str(v) for v in non_null}
        except TypeError:
            distinct_values = set()
        ...
        if non_null_count > 0:
            if data_category == DataCategory.NUMERIC:
                distribution, value_range, percentiles, frequency, outlier_count, outlier_boundaries = (
                    self._profile_numeric(non_null)
                )
            elif data_category == DataCategory.CATEGORICAL:
                distribution, frequency = self._profile_categorical(non_null)
            elif data_category == DataCategory.TEMPORAL:
                distribution, value_range, percentiles, frequency = self._profile_temporal(non_null)
    ...
    def _check_normality(self, values: np.ndarray) -> dict[str, Any]:
        ...
        try:
            ...
            _logger.debug("Shapiro-Wilk normality test failed")
        ...
        if len(values) >= 20:
            try:
                ...
                _logger.debug("DAgostino-Pearson normality test failed")
        ...
        try:
            ...
            if hasattr(ad_result, "pvalue") and ad_result.pvalue is not None:
                ...
                if (
                    len(ad_result.critical_values) > 2
                    and ad_result.statistic < ad_result.critical_values[2]
                ):
                    normal_votes += 1
        except Exception:
            _logger.debug("Anderson-Darling normality test failed")
        ...
        for dist_type, dist_obj in DISTRIBUTION_CANDIDATES:
            try:
                ...
                if dist_type in (DistributionType.LOG_NORMAL, DistributionType.GAMMA,
                                 DistributionType.WEIBULL) and np.any(values <= 0):
                    continue

                # Beta requires data in (0, 1).
                if dist_type == DistributionType.BETA and (np.any(values <= 0) or np.any(values >= 1)):
                    continue
...
def _to_numeric_array(values: list) -> np.ndarray | None:
...
def _temporal_to_epoch(values: list) -> np.ndarray | None:
...
def _fit_poisson(values: np.ndarray) -> float | None:
    ...
    try:
        ...
        _chi2_stat, p_value = scipy_stats.chisquare(obs_merged, f_exp=exp_merged)
        return float(p_value)
    except Exception:
        return None


def _compute_char_classes(str_values: list[str], lengths: list[int]) -> dict[str, float]:
    """Return character-class distribution (alpha, digit, special) ratios.

    Args:
        str_values: Cleaned string values.
        lengths: Pre-computed lengths of each string.

    Returns:
        Mapping of class name to ratio, or empty dict when no characters.
    """
...
def _compute_common_affixes(
    str_values: list[str],
) -> tuple[list[str], list[str]]:
    """Return top-5 common prefixes and suffixes (3-character).

    Args:
        str_values: Cleaned string values.

    Returns:
        Tuple of (common_prefixes, common_suffixes).
    """
...
def _build_format_patterns(
    str_values: list[str], max_samples: int,
) -> list[str]:
    """Build anonymised format patterns from a representative sample.

    Characters are mapped to ``A`` (alpha), ``9`` (digit), or kept as-is
    (special).  Only distinct patterns are returned.
    ...
    Returns:
        List of anonymised format pattern strings.
    """
...
def _regex_from_format(fmt: str) -> str:
    """Convert an anonymised format pattern (``A``/``9``) to a regex.

    Args:
        fmt: Format pattern string, e.g. ``"AAAA-9999"``.

    Returns:
        Anchored regex pattern, e.g. ``"^[A-Za-z]{4}\\-\\d{4}$"``.
    """
...
def _compute_pattern_metadata(values: list, max_samples: int = 5) -> PatternMetadata:
    ...
    char_classes = _compute_char_classes(str_values, lengths)
    common_prefixes, common_suffixes = _compute_common_affixes(str_values)
    sample_formats = _build_format_patterns(str_values, max_samples)

    regex_pattern: str | None = None
    format_pattern: str | None = None
    if sample_formats:
        format_pattern = sample_formats[0]
        regex_pattern = _regex_from_format(format_pattern)
...
def _cramers_v(col_a: list[str], col_b: list[str]) -> float | None:
...
def _mutual_information(col_a: list[str], col_b: list[str]) -> float | None:
